...
// RCM can be costly for graphs larger than 4 million,
// Degree Sort (Descending) may be better for load balancing.
    if (n > 4000000) {
        std::cout << "[RCM] Large graph detected. Using Degree Sort (DESC)..." << std::endl;
        rcm_order.resize(n);
        std::iota(rcm_order.begin(), rcm_order.end(), 0);

// Prioritize those with higher ranks (for Load Balancing)
        std::stable_sort(rcm_order.begin(), rcm_order.end(), 
            [&degrees](int a, int b) {
